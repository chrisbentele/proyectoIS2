...
const {createProject, deleteProject, editProject, getProjectById, getProjects} = require('../api/projects')
let projectId;
let createdProject;
...
test('crear proyecto', async () => {
    const res = await createProject({projectName:'testProject2',estimation:"10", scrumMasterId:"998"})
    expect(Object.keys(res.data)).toEqual(expect.arrayContaining(['id','duracionEstimada','fechaInicio','fechaFinalizacion','estado','miembros','nombre']))
  ...
  test('editar proyecto', async () => {
    const res = await editProject({projectId:projectId, projectName:'newName', estimation:14, status:1})
  ...
  test('editar proyecto con valores erroneos', async () => {
    expect.assertions(2)
    await editProject({projectId:projectId, projectName:'newName', estimation:14, status:4}).catch(err => expect(err).toBeDefined())
    await editProject({projectId:projectId, projectName:'newName', estimation:-1, status:4}).catch(err => expect(err).toBeDefined())
  })
